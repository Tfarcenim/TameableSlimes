...
public class SlimeFollowOwnerGoal extends Goal {
    public static final int TELEPORT_WHEN_DISTANCE_IS = 12;
    private static final int MIN_HORIZONTAL_DISTANCE_FROM_PLAYER_WHEN_TELEPORTING = 2;
    private static final int MAX_HORIZONTAL_DISTANCE_FROM_PLAYER_WHEN_TELEPORTING = 3;
    private static final int MAX_VERTICAL_DISTANCE_FROM_PLAYER_WHEN_TELEPORTING = 1;
    private final TameableSlime tamable;
    ...
    public SlimeFollowOwnerGoal(TameableSlime pTamable, double pSpeedModifier, float pStartDistance, float pStopDistance, boolean pCanFly) {
    ...
    private boolean maybeTeleportTo(int pX, int pY, int pZ) {
        if (Math.abs(pX - this.owner.getX()) < 2.0D && Math.abs(pZ - this.owner.getZ()) < 2.0D) {
            ...
            this.tamable.moveTo(pX + 0.5D, pY, pZ + 0.5D, this.tamable.getYRot(), this.tamable.getXRot());
            this.navigation.stop();
            return true;
        }
    }
